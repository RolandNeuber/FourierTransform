	...
	public static class FourierTransformOperations
	{
		...
		public static Complex[] FastInverseDFT(IList<Complex> frequencyDomainSignal)
		{
			static Complex[] fastInverseDFT(IList<Complex> frequencyDomainSignal)
			{
				...
				{
					return frequencyDomainSignal.ToArray();
				}
				else
				{
		...
		public static Complex[] IterativeInverseDFT(IList<Complex> timeDomainSignal)
		{
			timeDomainSignal = BitReverse.ReverseBits(timeDomainSignal);
			//i is the offset of the elements (left and right) to perform the butterfly operation on
			for (int offset = 1; offset <= timeDomainSignal.Count / 2; offset *= 2)
			...
			for (int i = 0; i < timeDomainSignal.Count; i++)
				timeDomainSignal[i] /= timeDomainSignal.Count;
			return timeDomainSignal.ToArray();
		}
		...
		public static Complex[] ParallelIterativeInverseDFT(IList<Complex> timeDomainSignal)
		{
			timeDomainSignal = BitReverse.ReverseBits(timeDomainSignal);
			//i is the offset of the elements (left and right) to perform the butterfly operation on
			for (int offset = 1; offset <= timeDomainSignal.Count / 2; offset *= 2)
			...
			for (int offset = 1; offset <= timeDomainSignal.Count / 4; offset *= 4)
			{
				...
				for (int pair = 0; pair < offset; pair++)
				{
					...
					for (int section = 0; section < timeDomainSignal.Count; section += 4 * offset)
					{
						...
						Complex factor2 = new(Math.Cos(Math.PI / offset * pair * 2), Math.Sin(Math.PI / offset * pair));
						Complex factor3 = new(Math.Cos(Math.PI / offset * pair * 3), Math.Sin(Math.PI / offset * pair));
						Complex factor4 = new(Math.Cos(Math.PI / offset * pair * 4), Math.Sin(Math.PI / offset * pair));
						...
						timeDomainSignal[elem1] = timeDomainSignal[elem1] + factor[0, 1] * timeDomainSignal[elem2] + factor[0, 2] * timeDomainSignal[elem3] + factor[0, 3] * timeDomainSignal[elem4];
						timeDomainSignal[elem2] = timeDomainSignal[elem1] + factor[1, 1] * timeDomainSignal[elem2] + factor[1, 2] * timeDomainSignal[elem3] + factor[1, 3] * timeDomainSignal[elem4];
						timeDomainSignal[elem3] = timeDomainSignal[elem1] + factor[2, 1] * timeDomainSignal[elem2] + factor[2, 2] * timeDomainSignal[elem3] + factor[2, 3] * timeDomainSignal[elem4];
						timeDomainSignal[elem4] = timeDomainSignal[elem1] + factor[3, 1] * timeDomainSignal[elem2] + factor[3, 2] * timeDomainSignal[elem3] + factor[3, 3] * timeDomainSignal[elem4];
					}
				}
			}
			return timeDomainSignal.ToArray();
